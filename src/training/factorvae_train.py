...
class MLPEncoder(nn.Module):
    def __init__(self, input_dim: int, z_dim: int = 392, hidden=1024):
        super().__init__()
...
class MLPDecoder(nn.Module):
    def __init__(self, output_dim: int, z_dim: int = 392, hidden=1024):
        super().__init__()
        self.net = nn.Sequential(
...
def build_embeddings_from_pairs(
    ...
    compute embeddings using AutoTokenizer + AutoModel.
    Instead of concatenating embeddings from multiple fields, this version treats each field's embeddings
    as separate samples in the dataset.
    """
    print(f"[embed builder] Using HF model: {embed_model} on device {device}")
    tokenizer = AutoTokenizer.from_pretrained(embed_model)
    model = AutoModel.from_pretrained(embed_model).to(device)
    model.eval()

    # collect all text fields separately
    texts_all = []
    for f in text_fields:
        texts_all.append(ds_split[f])
    ...
    for i in tqdm(range(0, N, embed_batch_size), desc="Embedding batches"):
        ...
        # process each text field independently, add as separate embeddings
        for field_texts in texts_all:
            batch = [t if t is not None else "" for t in field_texts[i:end]]
            inputs = tokenizer(batch, padding=True, truncation=True, return_tensors="pt").to(device)
            with torch.no_grad():
                outputs = model(**inputs)
                emb = outputs.last_hidden_state.mean(dim=1)  # mean pooling
            embeddings_out.append(emb.cpu())

    # stack everything into one tensor dataset
...
def parse_args():
    ...
    p.add_argument("--z-dim", type=int, default=392)
    p.add_argument("--gamma", type=float, default=10.0)
    p.add_argument("--lr", type=float, default=1e-4)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--save-dir", type=str, default=None)
    p.add_argument("--embed-model", type=str, default="bert-base-uncased")
...
def main():
    ...
    save_dir = args.save_dir or f"models/factorvae_{args.task}_ckpts"
    os.makedirs(save_dir, exist_ok=True)
    set_seed(args.seed)
